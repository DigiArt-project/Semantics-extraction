...
print_folder_recurse() {
    for i in "$1"/*;do
        ...
        if [ -d "$i" ];then
            ...
            if ! [[ $categorie == *"/"* ]]
            then
                if [ ! "$categorie" = "training_esf" ]  && [ ! "$categorie" = "training_vfh" ]  && [ ! "$categorie" = "training_cvfh" ] && [ ! "$categorie" = "training_ourcvfh" ] && [ ! "$categorie" = "training_gshot" ]; then
        ...
        #Then for each folder, check if the file insite are file
            ...
            if contains "$i" "/$type_descriptor/" && contains "$i" "/$type_folder/" ; then
...
then
    ...
    printf "$0 dataset_path esf/vfh/cvvfh/ourcvfh/gshot ratio(nb training/testing. Defaut : 3) output type_folder (views/fulll)\n"
    exit 1
else
    ...
    print_folder_recurse $path $type_descriptor $ratio_count $type_folder
