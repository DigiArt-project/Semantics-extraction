...
then
    ...
    if [ -d "$1" ]; then
        ...
        if [ -z "$2" ]
            then
                echo "No extension given - Use defaut"
        fi

        if [ -z "$3" ]
            then
                echo "No output name given - Use defaut"
        else
            file_to_save=$3
        fi
