...
Utils::getCategories (const std::string &file_name, std::vector<std::string> &categories)
{
    ...
    while (!fs.eof ())
    {
        ...
        if (line.find("/") != std::string::npos) {
            ...
            if (!line.empty() && line.find("training_") == string::npos){
...
Utils::getDataFilename (const std::string &file_name, const std::string &category, std::vector<std::string> &data)
{
    ...
    {
        getline (fs, line);
        string category_exact = "/"+category+"/";
        if (line.find(category_exact) != std::string::npos && line.find("/") != std::string::npos) {
            data.push_back(line);
        }
...
Utils::upsampling(const pcl::PointCloud<PointType>::Ptr & cloud,float radius_search, float upsampling_radius,float upsampling_step_size){
    
    pcl::PointCloud<PointType>::Ptr filteredCloud(new pcl::PointCloud<PointType>);
    std::cout << "Before upsampling number of points : " << cloud->size() << std::endl;
    // Filtering object.
    ...
    filter.setSearchRadius(radius_search);
    ...
    std::cout << "After upsampling number of points : " << filteredCloud->size() << std::endl;
